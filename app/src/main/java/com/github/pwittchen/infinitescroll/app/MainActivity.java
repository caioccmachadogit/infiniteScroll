...
public class MainActivity extends AppCompatActivity {
  ...
  @NonNull private InfiniteScrollListener createInfiniteScrollListener() {
    return new InfiniteScrollListener(MAX_ITEMS_PER_REQUEST, layoutManager) {
      @Override public void onScrolledToEnd(final int firstVisibleItemPosition) {
        ...
        if(!allItemsLoaded){
          simulateLoading(new ICallBackAsync() {
            @Override
            public void callBack() {

              int start = ++page * MAX_ITEMS_PER_REQUEST;
              allItemsLoaded = start >= items.size();
              if (allItemsLoaded) {
   ...
   * @param iCallBackAsync
   */
  private void simulateLoading(final ICallBackAsync iCallBackAsync) {
